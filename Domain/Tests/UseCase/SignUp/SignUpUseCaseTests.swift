...
struct SignUpUseCaseTests {
    ...
    func testCheckUserSignUpRegisteredUser() async throws {
        // Given
        let repository = await MockSignUpRepository.registeredUser
    ...
    func testCheckUserSignUpNewUser() async throws {
        // Given
        let repository = await MockSignUpRepository.newUser
        let useCase = SignUpUseCase(repository: repository)
    ...
    func testCheckUserSignUpFailure() async throws {
        // Given
        let repository = await MockSignUpRepository.failure
        let useCase = SignUpUseCase(repository: repository)
    ...
    func testCheckUserSignUpProviderSpecificFailure() async throws {
        // Given
        let repository = await MockSignUpRepository.failsForProvider(Domain.SocialType.apple)
        let useCase = SignUpUseCase(repository: repository)
    ...
    func testSignUpUserGoogleSuccess() async throws {
        // Given
        let repository = await MockSignUpRepository()
        let useCase = SignUpUseCase(repository: repository)

        // When
        let result = try await useCase.signUpUser(
    ...
    func testSignUpUserFailure() async throws {
        ...
        // When & Then
        await #expect(throws: MockSignUpRepositoryError.self) {
            _ = try await useCase.signUpUser(
                accessToken: "invalid-token",
                socialType: Domain.SocialType.google,
    ...
    func testSignUpUserProviderSpecificFailure() async throws {
        // Given
        let repository = await MockSignUpRepository.failsForProvider(Domain.SocialType.google)
        let useCase = SignUpUseCase(repository: repository)
    ...
    func testSignUpUserWithDelay() async throws {
        // Given
        let repository = await MockSignUpRepository.withDelay(1.0)
        let useCase = SignUpUseCase(repository: repository)
        let startTime = Date()
